...
$id = (int)($_GET['id'] ?? 0);
$stmt = $pdo->prepare("SELECT l.*, c.name as category, u.name as author, u.email as author_email FROM listings l
...
// like/comment table handling (POST)
if ($user) {
    // Verify CSRF token for actions
    $csrfToken = $_POST['csrf_token'] ?? '';
    if (!verify_csrf($csrfToken)) {
        // Invalid CSRF - redirect with error message
        header('Location: view_listing?id='.$id.'&error=csrf');
        exit;
    }
      ...
      <div class="card-custom shadow-lg">
        <div class="p-6">
          ...
          <div class="flex items-center mb-3">
            ...
            <div>
              <strong>Contact: <?=htmlspecialchars($l['author'])?></strong><br>
              <a href="mailto:<?=htmlspecialchars($l['author_email'])?>" class="text-blue-600 hover:underline">
                <i class="bi bi-envelope"></i> <?=htmlspecialchars($l['author_email'])?>
              </a>
            </div>
          </div>
          <p class="text-gray-500 mb-0">
            <i class="bi bi-calendar"></i> Posted on <?=date('F j, Y \a\t g:i A', strtotime($l['created_at']))?>
          </p>
        </div>
      </div>
    </div>
