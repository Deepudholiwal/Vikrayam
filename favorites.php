...
$stmt = $pdo->prepare("SELECT l.*, c.name as category FROM listings l JOIN categories c ON l.category_id=c.id JOIN favorites f ON f.listing_id=l.id WHERE f.user_id=? ORDER BY l.created_at DESC");
$stmt->execute([$user['id']]);
$listings = $stmt->fetchAll();
require 'header.php';
?>
<div class="p-4">
  <h3 class="text-2xl font-semibold mb-4">My Favorites</h3>
  <?php if (empty($listings)): ?>
    <div class="bg-blue-100 border-l-4 border-blue-500 text-blue-700 p-4">You have not favorited any listings yet.</div>
  <?php else: ?>
